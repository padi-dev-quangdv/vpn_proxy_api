...
constructor(private val registerUseCase: RegisterUseCaseImpl) : ViewModel() {
    ...
    private fun validateRegister(fullName: String, email: String, password: String, confirmPassword: String): Boolean {
    ...
    fun onEvent(event: ViewEvent) {
        when (event) {
            is ViewEvent.RegisterEvent -> register(event.fullName, event.email, event.password, event.confirmPassword)
        }
    }
